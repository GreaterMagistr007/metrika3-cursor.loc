...
const handleSubmit = async () => {
  ...
  try {
    if (!showOtpInput.value) {
      // Запрос OTP для админа
      // TODO: Реализовать API вызов
      console.log('Запрос OTP для админа:', phone.value);
      showOtpInput.value = true;
      success.value = 'Код отправлен на номер ' + phone.value;
    } else {
      // Проверка OTP для админа
      // TODO: Реализовать API вызов
      console.log('Проверка OTP админа:', otp.value);
      success.value = 'Успешная авторизация!';
      
      // Перенаправление на админ дашборд
      setTimeout(() => {
        router.push('/admin');
      }, 1000);
    }
